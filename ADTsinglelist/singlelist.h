...
class SingleList {
    ...
    std::size_t lenght;
public:
    SingleList() : head(nullptr), tail(nullptr), lenght(0) {}
	...
	SingleNode<T> *node = new SingleNode<T>(other.front());
	head = node;
	tail = node;
	for(SingleNode<T> *cur=other.head->next ; cur != nullptr ; cur = cur->next){
    		node = new SingleNode<T>(cur->value);
		tail->next = node;
		tail = node;
    	}
	lenght = other.size();
    ...
    std::size_t size() const {return lenght;} // O(1)
